 ...
 * @author Huige1
 */
public class ActionPerformed extends Run
        {
    public void actionPerformed(ActionEvent event)
	{
		Object source=event.getSource();
		if(choose.getSelectedItem()=="SeeYouAgain.wav")// choose related lyric
		{
...
"Oh I'll tell you all about it when I see you again\n" +
"When I see you again");
                        chooseClip=sound1;//set audioclip as sound1
			chooseClip.stop();
			loop.setEnabled(true);	//loopping true.	    
		}
...
"I need to be the one who takes you home");
		...
		if(source==play)//clicked on play button
		{
			stop.setEnabled(true);
			chooseClip.play();//play selected audioclip
			chooseL.setText("Now Playing");
		}
		
		if(source==open)//clicked on open button
		{
			int select=mc.showOpenDialog(frame);//open computer files to add musics
			if(select==JFileChooser.APPROVE_OPTION)
			{
				File file=mc.getSelectedFile();//get selected file
				chooseL.setText(file.getName());
				choose.setSelectedItem(file.getName());
			}		
		}
		
		if(source==loop)//clicked on loopping button
		{
			looping=true;
			chooseClip.loop();
		...
		if(source==next)//clicked on next button
		{
		...
		if(source==last)//clicked on last button
		{
			if(choose.getSelectedItem()=="SeeYouAgain.wav")
			{
				choose.setSelectedItem("OneLastTime.wav");
		...
		if(source==stop)//clicked on stop
		{
			if(looping)
			{
				looping=false;
